...
void insertionsort_basic(void *start,void *end,unsigned long size,int (*cmp)(const void *,const void *)){
    void *ptr=start+size,*change,*temp=(void*)malloc(size);
...
void insertonce(void *start,unsigned long total_elem,unsigned long size,int (*cmp)(const void *,const void *)){
    if (total_elem<=1) return;
    void *ptr=start+size*(total_elem-1),*change,*temp=(void*)malloc(size);
    int check(const void *x){return cmp(x,temp)>=0;}
...
void insertionsort_skip(void *start,void *end,unsigned long skip_elem,unsigned long size,int (*cmp)(const void *,const void *)){
    void *ptr=start+size*skip_elem,*change,*temp=(void*)malloc(size);
